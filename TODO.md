...
- [x] CI/CD pipeline — minimal GitHub Actions workflow (`.github/workflows/ci.yml`): restore/build/test on push+PR to main/master, .NET 8, Release config, excludes the opt-in live-Dataverse `Category=Integration` tests (no creds in CI).
