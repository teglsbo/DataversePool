...
/// <see cref="DataversePool"/> should service the next acquire. v1 ships round-robin,
/// health-aware round-robin, and least-connections; this interface exists so further strategies
/// can be added without changing DataversePool's public API.
...
public interface ISlotSelectionStrategy
{
    ...
    /// tells the caller (<see cref="DataversePool"/>) whether <see cref="SlotSelection.Member"/>
    ...
    /// called by <see cref="DataversePool"/> right after every attempt (success or failure) -
    ...
    /// evidence the member is unhealthy). Called by <see cref="DataversePool"/> instead of
