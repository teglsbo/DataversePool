...
### Fix for #2: `AllUnavailableBehavior` (configurable, default unchanged)
`DataversePool` gets a new constructor parameter,
...
- **`FailFast`**: throw `DataversePoolUnavailableException` (with member names and — if known —
...
reasoning as `DataverseLease` in ADR-0008). `RoundRobinSlotSelectionStrategy` (no
...
indirect improvement #1 gets here is that `FailFast` + `DataversePoolUnavailableException` make it
...
- **Backward-compatible**: the default behavior (`FailOpen`) is identical to before this ADR. Existing
  consumers of `DataversePool`/the strategies are unaffected unless they themselves opt into
  `FailFast`.
...
- Test coverage: 52/52 passing after this change (Core 15, Dataverse 34, Polly 3), including 4 new
  `MemberCircuitBreakerTests` and 3 new `DataversePoolTests` (default fail-open, fail-fast,
